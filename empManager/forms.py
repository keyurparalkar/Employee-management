...
class EmpDetailsForm(forms.Form):
    ...
    emp_branch = forms.CharField(label="Employee Branch",max_length=256,required=True)
    ...
    fields = [email, fname, midname, lastname,emp_branch, gender, mob_no, alt_mob_no, marital_stat, blood_grp]
    _ = list(map(lambda elem: elem.widget.attrs.update({'class':'form-control'}), fields))
    
    dob.widget.attrs.update({'class':'form-control','placeholder': 'YYYY-MM-DD'})